...
time_0 = time.time()
# run_err_list = pd.read_csv('Data/run_errors_max_params.csv',header=None)[0].tolist()

save_name = 'Data/run_errors_max_params.csv'
...
for i_loop in range(len(DF_game_and_play)):
    ...
    # YAP_i.load_play()
    # YAP_i.calculate_max_params(save_data=True)
    try:
        YAP_i = op.YAP(game_ID_i,play_ID_i)
        YAP_i.load_play()
        YAP_i.calculate_max_params(save_data=True)
